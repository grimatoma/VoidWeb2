...
| 39 | **T4–T7 resource and recipe content** — T3 drilled per R77; T4 (Martian Reach, local shipbuilding) is the next-most-load-bearing drill. T5–T7 deferred. | GAME.md (Part I) Open Questions | Drill T4 next; T5–T7 after T0–T3 playtest. |
...
|---|----------|------------|-------------|
| R79 | NORTH_STAR pitch doc + locked direction answers | **NORTH_STAR.md added** at repo root as the load-bearing pitch document (separate from GAME.md). Locks: (a) Paragon Pioneers anchors **both** visual and gameplay; Anno 1800 secondary (gameplay only). (b) **Six** core systems with Surveying & Discovery promoted to peer (was setup-only flavor). (c) Short-session lens for pitch = 5–15 min typical; GAME.md's 1–3 min remains the *floor* — both framings compatible. (d) Monetization = **freemium-lite**: 100% playable free, sells cosmetics/convenience/sandbox/scenarios/Charter expansions; never sells speed, power, loot boxes, notification spam, or telemetry. (e) Player fantasy = competent expansion as VOID YIELD CO. founder; *the calmest possible space-empire fantasy*. (f) Eight visual concept-art scenes briefed (asteroid cold-open, Lunar Foothold, Map, mobile bottom-sheet hero, Mars Foothold, Cislunar automation graph, Saturn plateau, 5-star reviewer portrait) — unlocks Stage 0 art pass. (g) Success signal authored as a 5-star review voice ("Ravi, bus + bedside") to anchor the target audience. NORTH_STAR is the tiebreaker on player-fantasy questions; GAME.md remains the tiebreaker on mechanics. | NORTH_STAR.md; DESIGN_NOTES 2026-04-29 |
| R78 | Accessibility / Localization / Analytics commitment | **Deferred to post-prototype.** No v1 commitment — partial commitment without authoring is worst-of-both. Stage 3 playtest is the natural moment: at that point we have real screens to apply WCAG tokens, real strings to extract for L10n, and real player behavior to instrument. The deferral is explicit, not silent. Companion artifact: `BALANCE.xlsx` added at the repo root — 14-sheet designer-facing math-feel reference (Resources, Recipes, Adjacency, People Costs, Pop Tiers, Storage, Fuel, Ships, Tier Gates, Action Durations, Prefab Kits, T1–T2 Chain Math drill). | DESIGN_NOTES 2026-04-29; BALANCE.xlsx |
| R77 | T3 Cislunar Network drilled | **T3 content drilled.** 7 new resources (Rare Trace Elements, Ammonia, Logic Boards, Refined Hydrogen, Comms Module, Sensor Array, Maintenance Kit), 7 new recipes/buildings (Trace Mine, Logic Foundry, Comms Workshop, Sensor Lab, Maintenance Shop, Catalytic Reactor, Automation Hub), Automation Rules system (Maintain-Stock / Surplus-Export, optional Window-Preference), Networked pop tier (×2.25 People Capacity multiplier, 8h settle-in, Comms Module + Sensor Array drips), Automation Hub building (1 grid slot, hosts 4 rules, required for any automation at a body), 2 new Earth Prefab Kits (Cislunar Comms Relay, Sensor Network), pair-type extensions, T3→T4 gate concretized (Mars shipyard + Networked colony, with alternate-fulfillment per R76). Pair-type stacking cap retained (max 2). Closes T3 portion of Pending #39 (T4–T7 still deferred). | GAME.md (Part I) T3 Cislunar Network Drill |
| R76 | Tier gate alternate-fulfillment | **Added.** Each tier gate has 2 clauses (AND); each clause has primary + alternate path (OR within clause). T0→T1, T1→T2, T2→T3, T3→T4 all updated. Alternates designed equivalent-difficulty-different-approach (Iron Ore raw vs. Refined Metal sold; pop-on-multiple-habitats vs. fat single habitat; etc.) — never strict shortcuts. Solves the wait-at-199 wall pattern. Stage 3 acceptance: 10–25% of runs reach the alternate path. | GAME.md (Part I) Tier Gate Recipes |
| R75 | Multi-stop teaching moment in FTUE | **Added.** FTUE (t=0–15min) covers the core loop only. Three additional teaching beats fire on tier-up: T1 unlock = multi-stop logistics nudge (Mixer-1 + lunar habitat oxygen pickup); T2 unlock = cargo class differentiation nudge (Tankers vs. Haulers stat compare); T3 unlock = automation rules nudge (Maintain-Stock for oxygen). Each beat is one banner with deep-link CTA, skippable, dismissable. Bridges FTUE-skippable to mature mid-game where each tier brings a paradigm-shifting capability. | GAME.md (Part I) First 15 Minutes — Continued Teaching Beats |
| R74 | Fuel as real per-body constraint + storage cap semantics | **Speced.** Fuel deducts from origin body's tank at dispatch (per leg for multi-stop). Origin without fuel → route can't dispatch (alert). Mid-route leg without fuel → ship strands at next-leg origin until tanker arrives. Per-route base costs by leg distance (Earth↔Moon 3, Earth↔NEA 5, Earth↔Mars 25-40 depending on window, Earth↔Belt 60, Earth↔Jovian 140). Storage cap semantics: each Silo/Tank/Cryo holds **single resource at a time** (assigned at placement; reassignable when empty for $200 reset). AFK earnings cap on fuel availability per body now grounded. | GAME.md (Part I) Fuel Logistics Model |
| R73 | People Capacity speced | **Speced.** Earth-orbit starter crew = 8 people (free, T0). Each colony contributes Pop × pop-tier multiplier (Survival ×1.0 → Networked ×2.25). Per-building people_cost values for all T0–T3 buildings (storage = 0). Default behavior: auto-distribute priority-ordered by recipe margin; per-recipe lock toggle available. Under-allocation → linear cycle-rate scaling (50% staff → 50% output). Over-allocation guarded — new placements steal from auto-buildings, never locked. Suspended pop = 0 capacity. | GAME.md (Part I) People Capacity |
| R72 | Adjacency math tightened + layout-decision floor | **Updated.** Adjacency bonus range bumped from 10–25% to **15–35%**, with signature pairs (Mine + Smelter, Glass Furnace + Silicates Mine, Carbon Mill + Carbon Mine) at the high end. Pair-type table authored (T0–T2, T3 extension). Stacking cap: max 2 pair bonuses per building. **Layout-decision floor rule:** every grid spec must force a meaningful omission — player should not be able to fit "everything they can think of" plus full storage on a body. Stage 3 acceptance: a fully-built body hits a slot wall before a credit wall. Supersedes the 10–25% magnitude in R30 / R65 (range character preserved; numbers updated). | GAME.md (Part I) Resources and Recipes / Adjacency Pair-Type Table |
