...
## Creative Concept Prompts (exploratory)

> **Status: NOT part of the locked spec.** These five prompts explore areas the locked spec deliberately defers (cinematic moments, popovers, late-game scale, recap surfaces, alternate visual flair). Use them as design probes for conversations about future direction. Don't ship from these without first running the corresponding decision through `DECISIONS.md`.

The five concepts below have been mocked once already (see `concepts/proposed/16-*.png` through `20-*.png`). The prompts are kept here so they can be re-run through Claude Design with variations.

### 16. Tier-Up Cinematic (Stage-2 polish target)

```
Generate a CINEMATIC tier-up moment for T0 → T1 Lunar Foothold. This is the Stage-2 polish target that the v1 plain modal will eventually evolve toward (decision R19). The brief here is more theatrical than the rest of the spec — but still terse-corporate in voice.

LAYOUT (full-screen takeover):
- Wide letterbox frame top + bottom (cinematic 2.39:1 ratio inside the screen). Outside the letterbox is pure black.
- Hero canvas inside the frame: dark space scene, the Moon dominant lower-left, Earth small upper-right with a thin route arc connecting the two (cyan).
- Habitat module shown mid-deployment on the lunar surface — three connected pods, one with a soft pulse light.
- Reveal sequence (single moment, all visible at once for the screenshot):
  - Top-left: small status crawl "LAUNCH — D 014 04:38 — LUNAR FOOTHOLD ESTABLISHED" in monospace.
  - Center: large title "T1 — Lunar Foothold" in serif or stencil display face (departure from default Inter for cinematic-only treatment).
  - Bottom-left: 3-line briefing in terse-corporate voice — same flavor text as the v1 plain modal.
  - Bottom-right: minimal "Begin" CTA, no other chrome.
- NO unlock list visible — that's surfaced in the post-cinematic screen. The cinematic earns 5 seconds of dwell time, no more.

VOICE: terse-corporate, but allow ONE poetic line. E.g., "First permanent presence beyond Earth orbit." That's the polish budget.

TONE: optimistic hard sci-fi. Measured, awe but no melodrama. Reference: Apollo program publicity stills, not blockbuster movie posters.

Show me 2 variants — (a) typography-led with minimal illustration, (b) illustration-led with smaller type.
```

### 17. Sources & Sinks Popover

```
Generate the Sources & Sinks popover — opens on hover (desktop) or long-press (mobile) on any resource chip in the top status bar or any production node. Currently unresolved (DECISIONS P0 — popover scope).

LAYOUT (compact popover, ~340px wide, ~480px tall on desktop; full-width sheet on mobile):
- Header: resource name + current quantity + cap (e.g., "Refined Metal · 38 / 120 · 32%"). Chip showing trend (+4.0/min).
- Body has 3 stacked sections:
  1. Sources (producers): list of buildings producing this resource, with rate. Each row: building name + body + rate + status chip.
     - Smelter ×1 (NEA-04) · 4.0 /min · ok
  2. Sinks (consumers): list of routes/buildings/contracts consuming this resource, with rate.
     - Earth route × Hauler-1 · 2.4 /min · cyan in-window
     - Earth contract sale · 1.6 /min · standing order
  3. Time-to-cap / time-to-empty readout: based on net rate. Includes hint about what changes if a stall is resolved.
     - Net +0.0 /min · at cap → throughput stalled
     - Resolve "NEA-04 storage at cap" → +4.0/min net, cap clears in 8m
- Footer: "Open in Map · Open in Production" link buttons.

CONTENT specifics:
- Show concrete numbers, not "estimated" or "approximately".
- The "what changes if stall resolved" readout is the meaty bit — that's what makes the popover worth opening vs. the inline KPI.
- Popover anchors to the resource chip with a small caret.

VOICE: terse-corporate. No exclamation. "ok" lowercase.

Show me 2 variants — (a) compact with horizontal mini-charts of rate, (b) richer with a 24h sparkline.
```

### 18. Multi-System Overview (T5+ late-game scale)

```
Generate the late-game system overview that appears at T5+ (Jovian Reach) and beyond. The locked spec only details T0–T2; this concept tests whether the visual language scales when the player has 6+ colonies, 30+ ships, and 80+ active routes spanning the whole solar system.

LAYOUT (full screen, dark canvas):
- Wide orbital map with the Sun at upper-left, planets along an orbital sweep extending to lower-right (Mars, Asteroid Belt, Jupiter, Saturn).
- Each colonized body shows a small colony chip with population, tier badge (T1-T7), and a single "alert state" dot.
- Route arcs are thin and abundant. Cyan = good window, amber = poor, red = stalled. The visual challenge is making 80 arcs legible — try grouping by destination cluster.
- Region labels: "Inner system" (Earth+Moon+NEA), "Belt sector", "Jovian sector", "Saturnian sector".
- Top filter bar: "Show: All / Trade routes / Survey probes / Colonies in pinch" — segmented control.
- Side panel (right): "System digest" with KPI tiles: Total throughput (+$48k/h), Population (4,820 / Cap 5,200), Active routes (84), Pinch alerts (3 colonies in shortage), Pending tier-ups (2 ready).
- Bottom strip: 5 most recent events as a horizontal feed ("Belt-3 storage at cap", "Saturn-1 founded", etc.), each with timestamp.

CONTENT:
- 7 colonies across 4 sectors; 3 in pinch (amber), 1 in shortage (red), 3 stable (green).
- 1 prestige milestone visible: "T7 — System Corp eligible in 2/3 gates".

VOICE: terse-corporate. Population numbers tabular, currency leading.

Show me 2 variants — (a) heavily abstracted "subway map" route layer, (b) realistic-orbit layout with thin opacity arcs.
```

### 19. Weekly Recap Dashboard

```
Generate the Weekly Recap surface — a long-arc check-in dashboard that complements the AFK Return modal but covers a 7-day window instead of a single absence. Triggered manually via Settings → Recap, or surfaced automatically once per week if the player has logged in at least 3 times that week. NOT part of the locked spec — exploring whether the hybrid session cadence justifies a dedicated long-arc surface.

LAYOUT (full-screen page, light surfaces):
- Header: "Week of D 008 – D 014 · 6 sessions · 18h 24m active"
- Hero KPIs (4 tiles): Net credits this week (+$32,140), New ships built (3), Colonies grown (1), Milestones cleared (4).
- Credits trend chart (line, 7 day x-axis): area chart with cyan fill, hovering markers on session boundaries.
- Throughput table: Resource × Avg/min × Peak × Total this week × vs. last week (delta with arrow). 6 rows for primary resources.
- "What you fixed" list: 5 user actions that resolved alerts, ranked by impact ("Resolved Hauler-1 idle saved ~$840", "Imported O2 to First Habitat saved 0 suspended pop").
- "What you missed" list: 3 things that stalled while away ("NEA-04 storage at cap 14h cumulative this week").
- Suggested next focus card (single CTA, more of a "coach" tone than the rest of the UI): "T1 Lunar Foothold gate — sell 76 more Refined Metal to Earth. ~38m at current rate." [Open Trade] / [Mute suggestion]

VOICE: terse-corporate but with the suggested-focus card allowed slightly more warmth (still no exclamation, still numbers leading).

CONTRAST WITH AFK MODAL: AFK = "what happened in the last 4 hours, what's broken NOW". Weekly = "what's the trend, what should you focus on next."

Show me 2 variants — (a) one-page dashboard with all sections visible, (b) tabbed view (Trends / Wins / Misses / Focus).
```

### 20. Map Heat Mode

```
Generate the Map screen in HEAT MODE. The locked spec mentions Heat as one of 4 map modes (Default / Survey / Routes / Heat) but only specifies Default. This concept proposes a concrete shape for Heat.

LAYOUT (same chrome as Map Default — status bar, nav rail, alert rail, fleet strip):
- Map mode selector: Heat is now selected.
- Canvas: same Earth + Moon + NEA-04 layout, but bodies now tinted by net throughput:
  - Green tint = surplus producer (NEA-04 producing Refined Metal at 4.0/min, surplus +2.4/min after route demand).
  - Amber tint = balanced (Earth, sinks balance sources).
  - Red tint = deficit (Moon, life-support draw exceeds local production).
- Route arcs colored by saturation:
  - Cyan = healthy load (route 60-90% utilized).
  - Amber = under-used (<40%).
  - Red = saturated (>95%, throughput cap hit).
- Hover (or focused) body shows mini-popover with throughput breakdown: "NEA-04 · Net Refined Metal +2.4/min · 1 of 1 export route · 1 hauler".
- Side legend (right edge): explains the color coding. Compact, 3 rows.
- Top-of-canvas KPI strip: "System throughput: Refined Metal +6.4/min · O2 −2.1/min · Hydrogen +1.5/min" — quick at-a-glance.
- The right-stack (Selected) area shows a Heat-specific inspector: "Suggested rebalances" — 2 ranked suggestions ("Add Hauler to NEA-04 → Earth: clears 1h 42m of stalls/wk", "Reduce Lunar Mine to 50%: saves 0.4 power/min").

CONTENT:
- Make the deficit on Moon visually salient — soft red glow.
- Surplus on NEA-04 = soft green glow with a subtle outflow arrow indicator.

VOICE: terse-corporate. Suggestions phrased as actions ("Add Hauler"), not advice ("Consider adding").

Show me 2 variants — (a) tints applied directly to body sprites with route arcs in muted color, (b) tints rendered as concentric "thermal halos" around bodies and route arcs heavily colored.
```
